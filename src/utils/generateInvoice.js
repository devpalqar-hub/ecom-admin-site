...
export async function generateInvoice(order, lang = "en") {
    const doc = new jsPDF();
    ...
    /* ================= LOGO ================= */
    try {
        const logo = await loadImageAsBase64(`${import.meta.env.BASE_URL}raheeb-logo.jpg`);
    ...
    /* ================= BILLING ================= */
    doc.setFontSize(12);
    const billingTitle = lang === "ar"
        ? ar(labels.ar.billing)
        : labels.en.billing;
    doc.text(billingTitle, lang === "ar" ? 190 : 14, 46, { align: lang === "ar" ? "right" : "left" });
    doc.setFontSize(10);
    ...
    billingLines.forEach((line, i) => {
        ...
        doc.text(text, lang === "ar" ? 190 : 14, 54 + i * 6, { align: lang === "ar" ? "right" : "left" });
    });
    /* ================= ITEMS TABLE ================= */
    autoTable(doc, {
        startY: 90,
        ...
        body: order.items.map((item) => [
            ...
            item.quantity,
            `QAR ${(Number(item.product.discountedPrice) * item.quantity).toFixed(2)}`,
        ]),
    });
    /* ================= CALCULATIONS ================= */
    const itemsTotal = order.items.reduce((sum, item) => sum +
        Number(item.product.discountedPrice) * item.quantity, 0);
    ...
    const row = (label, value) => {
        doc.text(lang === "ar" ? ar(label) : label, rightX, y, { align });
        doc.text(lang === "ar" ? ar(value) : value, rightX + (lang === "ar" ? -40 : 40), y, { align });
        y += 6;
    };
    row(labels[lang].itemsTotal, `QAR ${itemsTotal.toFixed(2)}`);
    if (order.coupun) {
        row(`${labels[lang].coupon} (${order.coupun.couponName})`, `- QAR ${couponDiscount.toFixed(2)}`);
    }
    row(labels[lang].subtotal, `QAR ${subtotalAfterDiscount.toFixed(2)}`);
    row(labels[lang].shipping, `QAR ${shippingCost.toFixed(2)}`);
    row(labels[lang].tax, `QAR ${taxAmount.toFixed(2)}`);
    /* ================= FINAL TOTAL ================= */
    doc.setFontSize(12);
    doc.text(lang === "ar"
        ? ar(`${labels.ar.totalPayable}: QAR ${finalTotal.toFixed(2)}`)
        : `${labels.en.totalPayable}: QAR ${finalTotal.toFixed(2)}`, rightX, y + 8, { align });
    /* ================= SAVE ================= */
